...
const balances = await client.stride.exchange.v1beta1
    .exchangeBalances()
```

### Composing Messages

Import the `stride` object from `stride`. 
...
const {
    ...
    deposit
} = stride.exchange.v1beta1.MessageComposer.withTypeUrl;
```

#### CosmWasm Messages

```js
import { cosmwasm } from "stride";
...
```js
import { getSigningstrideClient } from 'stride';
...
const rpcEndpint = 'https://rpc.cosmos.directory/stride'; // or another URL
...
Contract schemas live in `./contracts`, and protos in `./proto`. Look inside of `scripts/codegen.js` and configure the settings for bundling your SDK and contracts into `stride`:
